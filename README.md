...
**v0.0.3 includes Local Freeform Archive for the exact verified Freeform 4.5 schema alongside the existing editable export.** The export demo uses a genuine supported Freeform 4.5 text selection; it does not imply support for every object. Freeform 4.5 version-7 data is not generally supported. Only the fixture-backed table and single-object sidecar fallbacks described below are recovered. See the [support matrix](#support-matrix) and [fidelity report](docs/fidelity.md) before importing real work.
